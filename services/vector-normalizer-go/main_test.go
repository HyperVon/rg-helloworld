...
func TestVersionCommand(t *testing.T) {
	...
	if !strings.HasPrefix(stdout.String(), "vector-normalizer ") {
		t.Fatalf("stdout = %q, want vector-normalizer prefix", stdout.String())
	}
	if stderr.Len() != 0 {
...
func TestMainInvocation(t *testing.T) {
	...
	if !strings.HasPrefix(string(out), "vector-normalizer 0.0.0-skeleton\n") {
		t.Fatalf("main() output = %q, want prefix %q", out, "vector-normalizer 0.0.0-skeleton\n")
	}
}
