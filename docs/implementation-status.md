...
| 2026-08-08 | Entry points: `rghw.sh --quiet/--fresh` | PASS (`rghw.sh --quiet` stdout `HELLO WORLD` only stderr empty via `RGHW_QUIET_FLAG`, `--dry-run --quiet` 12 bytes, `--fresh` kills forwards + `kubectl delete namespace` before bring-up, `bash -n` + `pre_commit_check.sh` green, `say/ok/warn` → `&2` + quiet-gated) |
