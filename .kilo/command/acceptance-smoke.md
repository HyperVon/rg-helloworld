...
- Read `AGENTS.md`, `docs/runbook.md`, and the `rghw-milestone` skill
  first.
- Do not read or copy `.local/`, `.env` files, kubeconfigs, MinIO
  credentials, database files, pre-existing logs, home-directory files, or
  any external runtime data.
...
- Start long-lived steps (cluster creation, wait loops) as tracked background
  processes and poll their bounded output for readiness; never leave the user
  waiting on a foreground command that never exits.
