...
func run(args []string, stdout, stderr io.Writer) int {
	...
	fmt.Fprintln(stderr, "rghello: unknown command")
	fmt.Fprintln(stderr, "usage: rghello run [--message TEXT] [--api-url URL] [--timeout DURATION] [--quiet]")
	return exitInvalid
}
...
func runVersion(args []string, stdout, stderr io.Writer) int {
	...
	if len(args) > 0 {
		fmt.Fprintln(stderr, "rghello: version ignores extra arguments")
	}
	return exitSuccess
}
